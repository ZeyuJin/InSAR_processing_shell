...
# run_correct_subswath.sh $MATLAB up_h.grd 30 2000
# gmt grdedit ph_correct.grd -T -Gup_h.grd
# 
# run_correct_subswath.sh $MATLAB up_l.grd 30 2000
# gmt grdedit ph_correct.grd -T -Gup_l.grd
# 
# run_correct_subswath.sh $MATLAB up_o.grd 30 2000
# gmt grdedit ph_correct.grd -T -Gup_o.grd
...
run_correct_subswath.sh $MATLAB tmp_ph0.grd 30 220  # remove the discontinuity at the boundary
gmt grdedit ph_correct.grd -T -Gtmp_ph0.grd        # convert the gridline node to pixel node
rm -f ph_correct.grd
# exit 1

# gmt grdfilter tmp_ph0.grd -Dp -Fm21/21 -Gtest.grd -V -Nr
# mv test.grd tmp_ph0.grd
...
foreach iteration (1 2 3) 
  ...
  if ($odd == 1) then
    ...
    # gmt grdfilter tmp_ph_interp.grd -Dp -Fg$filtx/$filty -Gtmp_filt.grd -V -Ni -I$filt_incx/$filt_incy
    run_gauss15x3_filter.sh $MATLAB $filtx $filty  # anistropic gaussian filter instead of rectangular filter
    gmt grdedit ph_filt.grd -T -Gtmp_filt.grd
  endif
  ...
  # run_linear_extrapolate.sh $MATLAB tmp.grd tmp2.grd
...
# run_linear_extrapolate.sh $MATLAB tmp_ph.grd ph_filt.grd $filtx $filty
# gmt grdedit ph_filt.grd -T -Gtmp_filt.grd
