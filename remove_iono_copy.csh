...
if ($#argv < 2) then
   echo ""
   echo "remove_iono_copy.csh dates.pair config_file [ncores]"
   echo ""
   exit 1
endif
...
mkdir -p iono_larger_correction
cd iono_larger_correction
cp ../$1 .
...
foreach pair (`cat $intf_file`)
   ...
   if ($iono_skip_est == 0) then
     echo "estimate_ionospheric_phase_linear.csh ../../intf_h/$pair ../../intf_l/$pair ../../intf_o/$pair ../../$pair $iono_filt_rng $iono_filt_azi >& iono_$pair.log" >> estimate_iono.cmd
